...
class ManagerInfoActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        try {
            ...
            changePasswordButton.setOnClickListener {
                ...
                if (newPassword.isNotEmpty()) {
                    ...
                    if (!Pattern.matches(passwordPattern, newPassword)) {
                        ...
                        updatePassword(managerCF, hashedPassword)
                    }
                } else {
            ...
            Log.e("ManagerInfoActivity", "Error in onCreate: ${e.message}")
    ...
    private fun updatePassword(cf: String, newPassword: String) {
        ...
        client.newCall(request).enqueue(object : Callback {
            override fun onFailure(call: Call, e: IOException) {
                Log.e("ManagerInfoActivity", "Failed to update password: ${e.message}")
                runOnUiThread {
            ...
            override fun onResponse(call: Call, response: Response) {
                runOnUiThread {
                    if (response.isSuccessful) {
                        ...
                        Log.e("ManagerInfoActivity", "Failed to update password: ${response.message}")
    ...
    private fun fetchManagerInfo(cf: String) {
        ...
        client.newCall(request).enqueue(object : Callback {
            override fun onFailure(call: Call, e: IOException) {
                Log.e("ManagerInfoActivity", "Network error: ${e.message}")
                runOnUiThread {
            ...
            override fun onResponse(call: Call, response: Response) {
                try {
                    ...
                    Log.e("ManagerInfoActivity", "JSON parsing error: ${e.message}")
                    runOnUiThread {
                        Toast.makeText(this@ManagerInfoActivity, "Error parsing server response.", Toast.LENGTH_SHORT).show()
                    }
                } catch (e: Exception) {
                    Log.e("ManagerInfoActivity", "Error in fetchManagerInfo: ${e.message}")
                    runOnUiThread {
                        Toast.makeText(this@ManagerInfoActivity, "An unexpected error occurred.", Toast.LENGTH_SHORT).show()
                    }
                }
            }
        })
